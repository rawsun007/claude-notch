...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "The notch tells you when Claude Code itself is out of date, and what the update contains. It reads the installed version, checks the npm registry for the current one, and works out how you installed it so the update command it offers is the right one for a native, npm or Homebrew install. The card carries the headline changes from the release you are missing, and stops asking once the update lands.",
        "An MCP server can ask you a question through the notch. Servers can stop mid-tool-call to ask something, and until now that question waited in a terminal window you were not looking at. It now arrives as the same card Claude's own questions use, and your answer goes straight back to the server. Only questions a card of buttons can answer honestly are taken: a yes or no, or a pick from a list. Free text, numbers and browser sign-in flows are left to Claude Code's own dialog, exactly as before, rather than half-answered. If the question is resolved anywhere else, or cancelled, the card comes down by itself instead of waiting five minutes for an answer nobody can give.",
        "A forked session is no longer indistinguishable from a resumed one. A fork is a second copy of a conversation you are probably still in, running in its own worktree and spending its own money, and in the list it looked exactly like a resume: same project, same branch, so the two rows read as one session drawn twice. Forks now carry a chip, shown only when the session's CLI is new enough to tell the two apart.",
        "Sessions show when they are at their subagent or web search limit. Claude Code lets a session run twenty agents at once and make two hundred web searches in its life. Both refusals arrive as an ordinary tool result: Claude reads it, carries on without the work it meant to delegate, and says nothing about it in the reply, so from the outside the session just does less, which reads as stuck. The agent badge now becomes a fraction once the cap is reached, and stays that way if a spawn was actually refused. The search budget gets the same chip in its last tenth. Raised limits in your settings are read, so a raised cap shows as a raised cap.",
        "Compaction has a real ending. The \"compacting context\" cue used to be lowered by whatever hook happened to arrive next, so it vanished early on a busy session and lingered on a quiet one. Claude Code now fires an event when compaction actually finishes, and the summary the session keeps in place of what it dropped goes into the history.",
        ]),
        ChangeGroup(kind: .fixed, items: [
        "The context bar reads from after a compaction, not before it. The meter took the most recent turn's occupancy as the live figure, and the last turn before a compaction is the fullest the window ever gets, so the bar sat in the red immediately after the compaction that had just emptied it. It now resets at the compaction boundary and waits for the next turn to report the real number. Cost keeps accumulating, since compaction does not refund the turns that led to it.",
        "A session is not shown a badge its Claude Code cannot produce. The sandbox posture is read from settings keys that only take effect from 2.1.219, so a session on an older build was described by a badge for a fence it was not standing behind. The badge now depends on the version that session is running, and its tooltip says which facts that build cannot report, because an older session going quiet otherwise just looks unsandboxed.",
