...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "The notch answers again the moment it restarts, instead of forty seconds later. Updating the app left it deaf for about that long every time, and Claude Code printed a connection error for every hook fired in the gap. The cause was ours, not macOS. The hook socket asked for no address reuse at all, on the reasoning that a listening socket never enters TIME_WAIT. True, and beside the point: every connection it accepts does, carrying the same port, so an app that had answered even one hook could not take its port back until the kernel let those go. Measured with twenty-eight of them outstanding, a restart now reaches the listening socket in 0.4 seconds. The setting that would let another program share the port stays off, which is the one that mattered for safety.",
        "The file holding the hook token is no longer world-readable. Installing the hooks writes a shared secret into your Claude Code settings, and neither the installer nor the uninstaller set permissions on what they wrote, so that file and its backups sat at whatever the system default was. Nothing could reach them, because the folder above is private, but that is the folder's doing rather than ours: a restored backup or a synced home would have exposed the token, and that token is what stops another program on your Mac from answering permission prompts in this app's place.",
        "Two more link-file types are revealed in Finder rather than opened. Clicking a file the agent edited opens it, and the check in front of that already refused folders, anything executable, and a list of formats that act on open instead of showing you something. The list covered .webloc and .url but not .inetloc and .fileloc, which are the same kind of file under different names, and the ones that have actually been used to run a command by pointing at the right URL.",
        "The site stopped serving last month's release notes. The changelog on the website had said 0.30.0 since the twentieth while two releases shipped, because the entries were being published to the repository that holds the site's source rather than the one that serves it.",
        ]),
        ChangeGroup(kind: .changed, items: [
        "Opening the site costs about a megabyte instead of ten. Every clip on the front page and the videos page was playing on load, so arriving downloaded the whole library whether or not you watched any of it. Tiles now show a still frame and fetch the clip when you point at one.",
        ]),
    ]
}
