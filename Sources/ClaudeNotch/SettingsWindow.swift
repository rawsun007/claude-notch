...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "A usage window that has already reset stops being reported as live. Claude Code only pushes a status line while a session is redrawing, so when you stop working the last reading simply sits there, and nothing ever invalidated it. The log on my own machine has a reading taken at 07:37 saying the window resets at 07:40, then nothing at all until 05:28 the next day: for twenty-two hours the app held a reset instant that had passed, and a past instant renders as the word \"now\". So the card said a window would reset now when it had reset hours earlier, which is precisely backwards for anyone deciding whether to wait it out. Expired windows are now dropped on every status line, on the twenty-second tick, and once at launch. Both halves go, not just the date: a percentage measured inside a window that ended says nothing about the one running now, so it reads as unknown until a real reading arrives.",
        "Installing no longer breaks the app's own code signature. The install stamped a fingerprint file inside the bundle so a no-op install could skip the swap, and it wrote that file after signing, which left every install with a seal that no longer verified. An invalid signature is worse than an unsigned one, and it matters most on somebody else's Mac, where it is what Gatekeeper refuses hardest. The stamp now lives outside the bundle and existing installs repair themselves. To be clear about what this did not fix: it does not shorten the delay before the app starts answering hooks after an update. That was the guess, and measuring afterwards contradicted it.",
        ]),
    ]
}
