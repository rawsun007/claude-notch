...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "Sessions say whether they are sandboxed. Claude Code can fence a session in: the filesystem is limited, network egress goes through an allowlist, and credentials can be masked so a command never sees the real secret. Whether a session runs that way is exactly the \"what is this agent allowed to do\" fact the notch exists to answer, and nothing showed it. A sandboxed session now carries a badge, amber rather than green when something is allowed to run outside the fence, with the allowed domains, masked credentials and exceptions in the tooltip.",
        "The notch shows what the sandbox actually blocked. The badge says a fence exists; a blocked host or file path now says when the agent walked into it, on a card and in the history, with a running count on the badge. A session hitting the fence a dozen times used to look like a session failing for no visible reason.",
        "Auto mode's refusals are visible. Auto mode decides both ways, and only one of them was ever shown: an approved action popped a card, a blocked one showed nothing, so a session being refused over and over looked exactly like a session thinking. Denials now get the same card in the other direction, with the reason underneath, counted separately from the ones you made yourself.",
        "Settings changes are announced while sessions are running. Settings files are where permissions, allow rules and sandboxing live, so an edit rewrites what your running agents may do. A change to your settings, the project's, the local ones or a managed policy now raises a card naming which file it was. The app stays quiet about its own writes, and about skills changes, which change what Claude knows rather than what it may do.",
        "Every running session appears, not just the ones that talk to us. The notch only knew sessions that fired a hook at it, so a session started before the app launched, or in a project whose settings never got the hooks, did not exist as far as the list was concerned. It now reads Claude Code's own registry of running sessions, which also means a session that ends is removed at once instead of sitting there until it has been quiet long enough to look dead. The CLI version each session is running is in its tooltip.",
        ]),
        ChangeGroup(kind: .fixed, items: [
        "The allow-rule export no longer writes permission forms Claude Code warns about. Claude Code 2.1.210 deprecated Write(path), NotebookEdit(path) and Glob(path) rules and prints a startup warning for each one it finds. Clicking \"always allow\" on a file and copying the rules into settings.json, exactly as the app suggests, left you with a file that nagged on every launch. Those rules now export as Edit or Read, which is the same permission spelled the current way. Tool-wide rules are untouched, since rewriting one would widen a rule you never widened.",
        ]),
    ]
}
