...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "Hooks now carry a shared secret, so not every program on your Mac can pretend to be Claude Code. The port this app listens on accepted anything that could reach it, which is every process running as you: enough to invent a session, fake activity, or put a card in front of you that looks like Claude asking and read whatever you answer. The hook URL now carries a token that the app checks. Stated plainly, because it is easy to oversell: a program determined to find the token can read it, since it lives in your settings file. What it stops is the casual version, another account on the same Mac, and anything that has not gone looking. The check turns itself on only when the URL Claude Code is posting to actually carries a token, so an install that has not been re-run keeps working exactly as before rather than going deaf.",
        "The notch says when your organisation has restricted this Mac, or is watching it. Claude Code keeps a managed-policy file that nothing surfaced. On a personal Mac this shows nothing at all. On a company one it answers the question this app exists for, one level up: not what an agent may do in this project, but what someone above you already decided, and whether these sessions are recorded. A monitoring notice is something you should be told plainly and once, not discover later.",
        "Sessions record which instruction files are shaping them. Every CLAUDE.md, imported memory file and glob match Claude Code pulls in is now listed against the session that loaded it. The interesting case is the file that arrives halfway through, because by then the startup output where it scrolled past is long gone.",
        "The task meter reads Claude Code's own task list from disk. The hooks only describe what happened while this app was listening; the directory describes what is true, which is what a session resumed after a restart needs. It also carries the dependencies between tasks, so one waiting on another can be told from one nobody has started. This does not bring the meter back on the newest models, which no longer have the to-do tools at all; it makes it right when there is something to be right about.",
        "The notch notices when the working tree moves under an agent. A file changing that Claude did not change is usually you editing in another window, a branch switch, or a build, and it is how an agent ends up reasoning about a file that no longer says what it read. Counted quietly rather than announced: a card for each one would be noise during an ordinary build.",
        ]),
        ChangeGroup(kind: .fixed, items: [
        "A usage limit that lifts without the task restarting is now reported. The resume notice is driven by the session doing something, which covers every case except the one that matters most: nothing happening is not an event, so nothing was watching for it. Ten minutes past the reset with no sign of life now raises a card, and it names both explanations, because from here they are indistinguishable: Claude Code was closed, or continuing automatically is switched off and the terminal is holding a dialog for you.",
        "A merge request is called a merge request. Claude Code resolves the open review for your branch and reports GitHub pull requests and GitLab merge requests through the same fields, so the data was already arriving and the chip called all of it a pull request, drawing \"#42\" beside a pull-request icon for something GitLab does not have. GitLab now gets !42 and a merge icon, and the host is read from the URL, so a self-hosted GitLab with nothing in its name is still recognised.",
        ]),
    ]
}
