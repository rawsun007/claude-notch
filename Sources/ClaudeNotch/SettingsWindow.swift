...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "The updater refuses to install anything it could not verify. It fetched the published checksum for the download, and if that one request failed it said so and installed anyway. Anyone able to serve a bad download can also make one request fail, so that was the whole check. It now stops and points at Homebrew instead. Two more in the same place: it deleted the installed app before copying the new one, so a copy that failed halfway left no app at all, and it installed whatever the disk image contained. The new copy is staged beside the old one and swapped, a failure at any step puts the previous version back, and the incoming app has to be signed by whoever signed the one it is replacing.",
        "ClaudeNotch waits for its port instead of giving up on it. The port is busy at exactly one predictable moment, while the copy being replaced finishes exiting, and every update and quick restart passes through it. Giving up on the first refusal left the app running deaf, receiving nothing and showing nothing, until somebody relaunched it by hand. It now keeps trying, takes the port within seconds of it coming free, and stays quiet about a conflict that resolves itself.",
        "Your Claude Code settings are no longer copied to disk on every launch. Installing the hooks is idempotent and runs by itself whenever a release starts listening to a new event, and it backed up settings.json regardless, with nothing to remove the copies. One machine here had 311 of them going back three months. They are not clutter: each is a full copy of a file that can hold tokens. The backup now happens only when something is actually about to change, the five newest are kept, and they are written so only you can read them.",
        "The hooks are installed by one piece of code again. They were merged into settings.json by the app and, when the app was not on disk yet, by a shell script doing the same job separately. The shell copy had fallen ten events behind, so anyone who set up before installing the app got a hook set from months earlier. Setup now asks the app to do it, the fallback registers exactly the same events, and a test compares the two lists on every build.",
        "A connection that is never answered no longer holds on forever. Nothing had required a hook to be answered, so a path that returned without replying kept its connection until the app quit. No path actually did that, but only because each of them remembered to.",
        ]),
        ChangeGroup(kind: .changed, items: [
        "Cards that are waiting for you cost the app nothing to hold. Each one used to occupy a thread for as long as it sat there, and there are only about sixty-four of those, so a busy machine running several sessions with subagents could tie up all of them, at which point everything else the app does in the background stops: no transcript reads, no cost updates, no further hooks. Eighty cards can now wait at once and an unrelated request is still answered in a hundredth of a second.",
        ]),
    ]
}
