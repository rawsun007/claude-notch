...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "Less crowding on a session row. The prompt-cache warning was wide enough to truncate itself, dropping the token count that was the only useful part of it, and is now a snowflake and the number. A background agent no longer says so twice: the Attach button already tells you what it is, so the plain AGENT chip beside it is gone. The chip still appears when an agent is blocked and waiting for you, which is a different thing and one nothing else on the machine will tell you.",
        ]),
