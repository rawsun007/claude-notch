...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "Another program on your Mac can no longer receive Claude Code's permission prompts. The hook server let a second process running as you bind the same port and take a share of the incoming hooks, which include the ones that block until they are answered. A program doing that could reply \"allow\" to every tool call while no card ever appeared here, which is the one thing this app exists to prevent. The listening socket is now bound directly, with no sharing options and only on the loopback addresses, so a second attempt is refused. Reproduced before the fix and after it, against real builds.",
        "The notch says when it is not receiving prompts at all. A hook server that could not start wrote a line to the system log and nothing else: the menu bar icon appeared, the notch behaved, and no cards arrived, which is indistinguishable from a quiet afternoon. It now shows a warning icon in the menu bar and raises a card that says what is wrong, names the port, and hands over the command that tells you which program has it.",
        "A connection that never finishes its request is closed. A local program could open a socket, send half a request and hold a file descriptor for as long as it liked; enough of those and the app runs out. Thirty seconds to send a complete request, after which the socket goes. Blocking cards are unaffected: they are supposed to sit there while you decide.",
        "Copy resume command no longer hands you an unquoted shell line. The session id in it comes from a hook payload or a transcript filename, so an id carrying a second command would have run it when you pasted, on this app's suggestion. It is quoted now, tested against a real shell.",
        "An MCP server cannot flood the elicitation card. Its message, field labels and options arrive from a third-party program you installed and had no length or count limit. Past a cap the question is declined and handed back to Claude Code's own dialog, rather than truncated, because a question nobody could read would still have returned an answer as if they had.",
        ]),
        ChangeGroup(kind: .added, items: [
        "A tool call that fails is no longer invisible. Claude Code reports failures and this app never listened, so a session whose every command was failing looked exactly like a session working: the only thing on screen was the name of the tool it had just started. Failures now go to the history, and three failures of the same tool in a row raise a card, because that is a session going in circles rather than a command that failed on purpose. Pressing Escape comes through the same channel and is not treated as a failure, since you already know.",
        "The app says why the task meter is empty. Claude Code 2.1.233 took the to-do tools away from its newer models, so sessions stopped reporting a checklist and the meter had nothing to draw. An empty meter and a broken one look the same. Settings now explains it and gives you the setting that turns it back on, but only when there is evidence for it: a session that has run twenty tools without ever mentioning a task, on a version new enough to have removed them.",
        "Ten more GitLab token formats are redacted. GitLab issues a family of prefixed tokens and this app knew one of them. It matters more here than in a terminal, because history entries are saved to disk and every export is built from them, so a token that slipped through outlived the session it appeared in.",
        ]),
    ]
}
