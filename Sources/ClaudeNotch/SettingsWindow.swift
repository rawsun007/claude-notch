...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "The app is notarized by Apple. Downloading it no longer produces the warning that Apple cannot verify it is free of malware, and there is no longer a trip to System Settings to allow it. Download, drag to Applications, open. That is the whole install now.",
        "Two things follow from that, both of which you would have noticed. The pause after every update, where the notch received nothing for the better part of a minute, is gone: it was macOS assessing an unsigned app, and there is no longer anything to assess. And because the app is signed with a stable identity rather than a fresh one each build, macOS stops treating each update as a new app, so Accessibility permission is asked once instead of again after updates.",
        "Releases can be built and notarized on CI. A release no longer depends on one machine's keychain or one person being awake, and the file people download is built from the exact commit its tag points at.",
        "The project opens in Xcode. tools/xcode.sh generates the project and opens it, for editing and debugging with breakpoints. build.sh is still what produces a release.",
        ]),
        ChangeGroup(kind: .fixed, items: [
        "The app can drive your terminal again under notarization. Signing for distribution requires the hardened runtime, and under it an app may not send events to another app without saying so up front. Nothing said so. The first notarized build would have shipped with resume, send message and compact silently unable to reach Terminal, with no error to explain it. Found while wiring up signing, fixed on the same commit.",
        "Clicking a file the agent edited no longer opens two kinds of file that act on open rather than showing you something. The check in front of that already refused folders, anything executable, and a list of formats that run on open. It named the risk exactly and still missed .inetloc and .fileloc, which are the same kind of link file as .webloc under other names.",
        ]),
    ]
}
