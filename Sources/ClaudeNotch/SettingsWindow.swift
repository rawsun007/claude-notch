...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "The notch says how much ran without anyone being asked. Auto mode has been the default since the middle of August: a classifier reviews each action instead of you, and it is better at catching a dangerous command than the people it replaced, by a wide margin. What nothing reports is the remainder, and Anthropic puts its miss rate on overeager actions at roughly one in six. After fifty actions in a session that never prompted you, the notch says so once. It makes the case for auto mode while it does it, because a card that only said the alarming half would be arguing against a setting that is safer than what it replaced.",
        "It tells you when a session changed code and never checked it. No tests, no build, nothing that could have failed. An agent stops when the work looks done, so without a check it can run, looking done is the only signal it had. Three edited files before it says anything, and once per session: a one-file change with an obvious diff is exactly where running the suite is overkill.",
        "It notices when two sessions are working in the same checkout. They edit the same files underneath each other and neither reports an error when it happens. Sessions already in their own worktree are left alone, since that is the thing being recommended. The card names the honest limit too: a worktree separates files, not ports, databases, or environment.",
        "It says when a CLAUDE.md has grown long enough to be skimmed, or emphasises so many lines that none of them stands out. Both bars sit well above the usual advice, because telling somebody their careful, correct file is a problem is the expensive way to be wrong.",
        "It points out an MCP server doing a job your machine already has a command for. A command costs nothing until it runs, while a server spends context on tool definitions every session, used or not. Only when that command is actually installed here: recommending one you do not have is homework, not advice.",
        ]),
        ChangeGroup(kind: .fixed, items: [
        "A build command had to start a line to count as one. Checking for a build anywhere in the text meant `echo make believe` counted as running one, so a session that had verified nothing could be treated as verified. Caught by its own test before it shipped.",
        ]),
    ]
}
