...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "A link can no longer start an agent behind your back. The claudenotch:// scheme lets Shortcuts, Raycast and Alfred drive this app, and one of its verbs resumed a session: a terminal opened and a conversation carried on, immediately, on the say-so of whatever opened the URL. A page you merely visit can open one, and the browser prompt asking whether to open this app is a habit rather than a decision. Resuming from a link is now a card you answer. It is marked dangerous, which says nothing about the session and everything about the caller: no always-allow rule and no auto-approve mode can answer it for you, and it takes a deliberate hold rather than a reflex click. AppleScript is left immediate, because macOS makes you grant one app permission to drive another explicitly and once, which is a real decision.",
        "Touch ID says why a confirmation did not go through. With the Touch ID gate on, that sheet is the only way to allow a dangerous command, and three unrecognised fingers locks biometrics until the Mac is unlocked with a password. From that moment the Confirm to Allow button did nothing at all, with no way to tell that from a slow sheet. The sheet now offers the system's own password option, and a failure that is not a cancel puts a line on the card saying what happened and what to do. Cancelling still says nothing, because that is an answer rather than a failure.",
        "A link that arrives before the app has finished starting no longer quits the app. macOS hands a URL to an app it has just launched for that purpose, so a link could land while the notch window and the menu bar were still being built, and reaching for them killed the process. It now does less instead.",
        ]),
        ChangeGroup(kind: .added, items: [
        "Settings answers whether hooks are actually arriving. Everything the notch shows comes from hooks Claude Code fires at it, none of that is visible, and when nothing appears there was no way to tell a quiet afternoon from a broken pipe. One line now says it: receiving events and how long ago the last one was, or nothing yet since the app started, or nothing for six hours, or the hooks are missing from your Claude Code settings, or the server is not listening at all. The two you can act on say what to do.",
        "The claudenotch:// scheme can be switched off. Its one privileged verb now asks first, but some people would rather the surface did not exist on their machine, and until now the only way to say so was not to install the app. Settings > Notch, on by default. Off means nothing a link asks for happens, not even a card.",
        ]),
    ]
}
