...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "Update Now proves the download came from us before it replaces the app. It checks that the new copy is signed by an Apple-issued certificate belonging to the same developer as the copy you already have, and that Apple notarized it, and refuses to install it otherwise. Until now it compared the name on the signature as text, which was the strongest check available while the app was self-signed: anyone can name a certificate whatever they like, and nobody outside the team can produce a bundle that satisfies the real check. This is the thing being replaced when you update, so it is worth it being the part that is hard to fake.",
        "Nothing we publish will ask you to click past Gatekeeper again. The install steps in the app, the README, the website and the Homebrew instructions all still walked you through Open Anyway, left over from before notarization. They are gone. A copy of ClaudeNotch that makes macOS complain is now a copy that did not come from us, and that is only useful as a signal if we never publish the workaround. To check any copy yourself: spctl -a -vvv /Applications/ClaudeNotch.app should say source=Notarized Developer ID.",
        ]),
        ChangeGroup(kind: .added, items: [
        "A release now refuses to publish rather than quietly shipping something Gatekeeper will block. Notarizing used to be skipped with a one-line note if the credentials were missing, which meant the tag, the Homebrew formula and the download could all go out around a file every user would be warned about. It stops before touching anything now.",
        "One command checks a build is actually distributable, and both the release script and CI gate on it. It asserts fifteen properties of the bundle: the signing identity and team, the hardened runtime, a timestamp that outlives the certificate, the entitlements, the permission prompt reasons, a stapled notarization ticket, and that the hooks, translations and URL scheme are all really inside. Signing can succeed and still leave the app broken in ways that produce no error at all, which is how a build with no entitlements at all nearly shipped.",
        ]),
    ]
}
