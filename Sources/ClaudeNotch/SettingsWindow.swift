...
struct SettingsView: View {
    ...
    static let whatsNew: [ChangeGroup] = [
        ...
        "Two new guest appearances for the pet. Iron-Pet hovers on its repulsors with the chest reactor lit and thrust flickering under its boots, and Stark-Pet is the man out of the armour, with the hair, the sideburns and the goatee, leaning out of the notch. Like Spider-Pet they are rare on purpose, turning up now and then among the everyday animations rather than on a schedule, and both are listed under Guest appearances in Pet Mode settings with the date they arrived, where you can play them on demand.",
        ]),
        ChangeGroup(kind: .fixed, items: [
        "A permission card no longer loses its buttons. A destructive edit with a long diff could be drawn short, with Deny and Confirm to Allow cut in half along the bottom edge. The card's measured height could be thrown away in the instant it changed from one card to the next and was then never measured again, so it fell back to an estimate that ran a few points short on every line of a diff. The measurement now belongs to the card it was taken from and cannot be lost that way, and the estimate itself now allows the real height of each diff line and of the warning banner.",
        ]),
    ]
}
